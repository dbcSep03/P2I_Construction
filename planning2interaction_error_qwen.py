...
def generate_fake_reasoning_qwen(
    user_msg: str,
    error_tool_call: Dict[str, Any],
    error_history: str,
    tool_call_info: str,
    encoding
) -> str:
    """生成模拟导致错误工具调用的虚假思维链（用于Qwen模型）"""
    prompt = f"""You are a **reasoning data generator** whose goal is to create high-quality reasoning traces that simulate how a model naturally reasons *toward* an incorrect tool invocation (not reflecting on it afterward).

The input consists of:
* **user_msg**: the user's natural language message
* **tool_call**: the current (incorrect) tool invocation
* **tool_call_info**: the information of all tools
* **tool_history** (optional): previous tool calls and their outputs, if multiple tools have been invoked in sequence

Your task is to generate a **coherent, step-by-step internal reasoning trace** that *leads naturally to the incorrect tool call*.

#### Output requirements:
* The reasoning must read as the model's **real-time thought process**, *not a post-mortem explanation*.
* The text should naturally **progress toward** the wrong decision, showing the assumptions, partial reasoning, and misinterpretations that cause the model to choose the given tool.
* The output must read like fluent, continuous inner thought. Do not include any post-mortem explanations or summaries.

#### user_msg:
{user_msg}

#### the current (incorrect) tool invocation:
{json.dumps(error_tool_call, ensure_ascii=False)}
...
{error_history}"""
    ...
    tokens = encoding.apply_chat_template(convo, add_generation_prompt=True, thinking=False)
    out = chat_json(tokens, max_tokens=4096)
    result = out.get("results", "")
    reasoning = ""
    if "</think>" in result:
        reasoning = result.split("</think>")[0].strip()
        result = result.split("</think>")[1].strip()
    return result.strip(), reasoning
...
def run_agent_tool_calls(agent_system_template: str,
                        ...
                        expected_responses_map: Dict[str, Any] = None,
                        ) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    """
    运行agent tool calls，使用multi-step逻辑（最多7次循环）
    支持 error_scenarios 注入：先将错误的思维链+工具调用+错误响应放入历史，再让模型修正。
    返回: (结果字典, 汇总消息列表)
    """
    ...
    # 兼容新格式（expected_right_tool_call/expected_tool_response）和旧格式（name/expected_tool_response）
    if expected_responses_map is None:
        expected_responses_map = {}
    if suggested_tools_detail and not expected_responses_map:
        for t in suggested_tools_detail:
            if isinstance(t, dict):
                if "expected_right_tool_call" in t and "expected_tool_response" in t:
                    name = t["expected_right_tool_call"].get("name", "")
                    if name:
                        expected_responses_map[name] = t["expected_tool_response"]
                elif "name" in t and "expected_tool_response" in t:
                    expected_responses_map[t["name"]] = t["expected_tool_response"]
    
    # 初始化tokens
    tokens = encoding.apply_chat_template(convo, add_generation_prompt=True, thinking=True)
    
    # 注入错误场景：生成导致错误的思维链 → 错误工具调用 → 错误响应，放入历史
    if error_scenarios:
        tool_call_info_str = render_tool_catalog(tools)
        error_history_text = ""
        for es in error_scenarios:
            error_call = es["error_tool_call"]
            error_resp = es["error_response"]

            # 1. 生成导致错误的虚假思维链
            fake_reasoning, fake_info = generate_fake_reasoning_qwen(
                user_msg, error_call, error_history_text, tool_call_info_str, encoding
            )
            all_messages.append({
                "role": "reasoning",
                "reasoning": fake_info,
            ...
            # 2. 将虚假思维链内容追加到 tokens（当前 tokens 末尾已是 <think>）
            reasoning_tokens = encoding.encode("\n" + fake_reasoning, add_special_tokens=False)
            tokens += reasoning_tokens

            # 3. 拼接错误的 tool_call（qwen_tool_call_template 会输出 \n</think>...tool_call...<|im_end|>\n）
            error_fc_list = [error_call]
            all_messages.append({
                "role": "tool_call",
                "content": error_fc_list,
                "reasoning": fake_reasoning,
                "is_error": True
            })
            add_tokens_error_call = [
                {"role": "tool_call", "content": '\n'.join([
                    f'<tool_call>\n{json.dumps(fc, ensure_ascii=False)}\n</tool_call>'
                    for fc in error_fc_list
                ])}
            ]
            error_call_tokens = encoding.apply_chat_template(
                add_tokens_error_call, add_generation_prompt=True, thinking=True,
                chat_template=qwen_tool_call_template
            )
            tokens += error_call_tokens

            # 4. 拼接错误响应（qwen_tool_response_template 会输出 <|im_start|>user...resp...<|im_end|>\n<|im_start|>assistant\n<think>）
            error_response_obj = {"name": error_call.get("name", ""), "results": error_resp}
            all_messages.append({
                "role": "tool_response",
                "content": error_response_obj,
                "is_error": True
            })
            add_tokens_error_resp = [
                {"role": "tool", "content": "<tool_response>" + json.dumps(error_response_obj, ensure_ascii=False) + "</tool_response>"}
            ]
            error_resp_tokens = encoding.apply_chat_template(
                add_tokens_error_resp, add_generation_prompt=True, thinking=True,
                chat_template=qwen_tool_response_template
            )
            tokens += error_resp_tokens

            error_history_text += (
                f"Tool call: {json.dumps(error_call, ensure_ascii=False)}\n"
                f"Error response: {json.dumps(error_resp, ensure_ascii=False)}\n"
            )

    # multi-step循环，最多10次
    max_loops = 10
    loop_count = 0
    all_tool_calls = []  # 收集所有的tool calls（仅正确的工具调用）
...
def run_from_planning(planning_data: List[Dict[str, Any]], out_dir: str, data_index: int, round_index: int) -> Tuple[Optional[str], Optional[List[Dict[str, Any]]]]:
    ...
    if not planning_data or len(planning_data) == 0:
        return None, None
    ...
    statistics = {
        ...
        "steps_stats": [],
        "total_retries": 0,
        "total_simulation_loops": 0,
        "total_assist_queries": 0,
        "steps_with_retries": 0,
        "steps_with_simulation_loops": 0
    }
    
    history_all = []
    world_state: Dict[str, Any] = {"cached": {}, "round_index": 0}
    error_occurred = False
    error_message = ""
    
    try:
        for idx, st in enumerate(plan_steps):
            # if idx > 0:
            #     import pdb; pdb.set_trace()
            world_state["round_index"] = idx
            ...
            history_hint = (
                f"Planning step {idx+1}/{len(plan_steps)}:"
                f"Step detail: " + json.dumps(step_detail, ensure_ascii=False)[:600]
            )
            ...
            # 解析新格式（expected_right_tool_call/expected_tool_response/expected_error_tool_call/expected_error_response）
            # 以及旧格式（name/expected_tool_response）
            expected_tools = []
            expected_responses_map = {}
            error_scenarios = []
            for t in suggested_tools_detail:
                if isinstance(t, dict):
                    if "expected_right_tool_call" in t:
                        # 新格式
                        right_call = t["expected_right_tool_call"]
                        tool_name = right_call.get("name", "")
                        if tool_name:
                            expected_tools.append(tool_name)
                        if "expected_tool_response" in t and tool_name:
                            expected_responses_map[tool_name] = t["expected_tool_response"]
                        if "expected_error_tool_call" in t and "expected_error_response" in t:
                            error_call = t["expected_error_tool_call"]
                            error_resp = t["expected_error_response"]
                            if error_call and error_resp:
                                error_scenarios.append({
                                    "error_tool_call": error_call,
                        ...
                        tool_name = t["name"]
                        expected_tools.append(tool_name)
                        if "expected_tool_response" in t:
                            expected_responses_map[tool_name] = t["expected_tool_response"]
                else:
            ...
            step_stat = {
                ...
                "success": False,
                "failure_reason": None,
                "has_error_scenario": len(error_scenarios) > 0
            }
            ...
            for retry_attempt in range(max_retries):
                ...
                try:
                    agent_out, agent_messages = run_agent_tool_calls(
                        ...
                        error_scenarios=error_scenarios if error_scenarios else None,
                        expected_responses_map=expected_responses_map if expected_responses_map else None,
                    )
                except Exception as e:
                    print(f"[警告] Step {idx+1} retry {retry_attempt} agent调用失败: {e}")
                    if retry_attempt == max_retries - 1:
                        # 最后一次重试也失败了
...
def main():
    ...
    parser.add_argument("--planning_json", type=str, default="plan_0208/ErrorResponse/res_1/valid_dialogues.json", help="Path to planning JSON file (for from_planning mode)")
    parser.add_argument("--out_dir", type=str, default="plan_0208/ErrorResponse/error_gen_1/merged_out")
    ...
    parser.add_argument("--batches_dir", type=str, default="plan_0208/ErrorResponse/error_gen_1/batch", help="Directory to store per-batch metadata JSONs")
    parser.add_argument("--dialogues_jsonl", type=str, default="plan_0208/ErrorResponse/error_gen_1/merged_dialogues_modified.jsonl", help="Append all dialogues into a single JSONL file")
